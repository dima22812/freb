...
@loader.tds
class FreebirMod(loader.Module):
	"""Афк модуль от Freebie с изменением био и имени
	...
	async def frebcmd(self, message):
		"""Конфиг модуля"""
		await self.allmodules.commands["config"](
					await utils.answer(message, f"{self.get_prefix()}config Freebie")
				)

	async def afk777cmd(self, message):
		"""Войти в AFK режим"""
		try:
			user_id = (
				(
		...
		await self.allmodules.log("afk777")
	...
	async def unafk777cmd(self, message):
		"""Выйти из режима AFK"""
